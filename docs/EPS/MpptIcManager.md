...
On each rate group tick in `RUNNING` state, `MpptIcManager` reads all relevant measurement and status registers over I2C, assembles the data into a battery state struct, and pushes it to `EPSApplication` via `batteryStateOut`.

When `EPSApplication` forwards a register-write command via the `setRegister` port, `MpptIcManager` writes the specified BQ25756E register over I2C and emits an event. When the BQ25756E INT interrupt fires, `MpptIcManager` reads the fault registers, emits a fault event, and re-initializes the IC by transitioning back to `UNINITIALIZED`.
...
Active component with an internal flat two-state F' state machine (`Fw::Sm`). The standard five-state hardware manager pattern is replaced by a two-state machine because BQ25756E initialization requires configuring the ADC and watchdog registers before normal measurement operation can begin.

### 4.2 Ports

| Port | Direction | Type | Purpose |
|------|-----------|------|---------|
| `schedIn` | Input | `Svc.Sched` | 1 Hz rate group tick; triggers measurement cycle in `RUNNING` state |
| `i2cWrite` | Output | `Drv.I2c` | Write BQ25756E registers; calls `LinuxI2cDriver.write` |
| `i2cWriteRead` | Output | `Drv.I2cWriteRead` | Read BQ25756E registers (write register address then read data in one repeated-start transaction); calls `LinuxI2cDriver.writeRead` |
...
> **Note:** There is no `icReset` GPIO port. The BQ25756E has no hardware RESET pin and no software reset register. Re-initialization is handled entirely by re-writing configuration registers on each `UNINITIALIZED` entry.
...
UNINITIALIZED (initial)
  └─ (entry) write REG_RST=1 (POW_PATH_REV_CONT 0x19 bit 7) — resets all registers to
              power-on defaults; bit self-clears. Ensures clean IC state before init sequence.
             read PART_INFO (0x3D) to verify device ID; emit WARNING_HI if mismatch
             configure watchdog: write TIME_CONT (0x15) to disable or set appropriate timeout
             configure ADC:
               write ADC_CONT (0x2B) — enable ADC, set continuous mode
               write ADC_CHANNEL_CONT (0x2C) — enable all channels
                 (VBAT, IBAT, VAC, IAC, TS, VFB)
             write charging configuration registers:
             ...
             read back key config registers; log WARNING_HI if any differ from written values
             → RUNNING
             emit "MpptIcManager transitioned to RUNNING" event

RUNNING
  └─ (on schedIn) read measurement registers (big-endian, 16-bit):
...
**I2C error handling:** Any I2C transaction returning a non-OK status logs `WARNING_HI` (throttled) and transitions to `UNINITIALIZED` (self-healing).

**`setRegister` during `UNINITIALIZED`:** Calls received while the IC is initializing are dropped with a `WARNING_LO` event noting the write was discarded. Attempting the write mid-init could conflict with the initialization sequence (which re-writes the same config registers), so dropping is safer than queueing. The operator must resend the command after the IC returns to `RUNNING`.

---

## 6. Notes

- **No hardware reset pin; software reset via REG_RST.** The BQ25756E has no hardware RESET GPIO. Software register reset is available: `REG_RST` is bit 7 of `POW_PATH_REV_CONT` (0x19). Writing 1 resets all registers to power-on defaults; the bit self-clears after reset. `CHARGER_CONT` bit 5 (`WD_RST`) is a separate, narrower operation that only resets the internal I2C watchdog timer — it does not reset IC state. On each `UNINITIALIZED` entry, `MpptIcManager` writes `REG_RST=1` first to clear any unknown IC state, then proceeds with the full initialization sequence.
